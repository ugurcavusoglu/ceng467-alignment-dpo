...
DPO improves semantic alignment (BERTScore F1: 0.8272) over the zero-shot baseline
(0.8303) and qualitatively reduces harmful outputs, demonstrating that preference
...
\midrule
Zero-Shot (TinyLlama-1.1B)   & 17.28 & 0.8303 & --- \\
SFT (5k, 2 ep.)              & \textbf{10.08} & 0.8234 & --- \\
DPO ($\beta$=0.1, 3k, 1 ep.) & 17.67 & \textbf{0.8272} & --- \\
\midrule
\multicolumn{4}{l}{\textit{Scaled results and preference accuracy to be updated}} \\
\bottomrule
\end{tabular}
\end{table}
...
dataset using TinyLlama-1.1B with LoRA fine-tuning. Initial results demonstrate that
DPO improves semantic alignment over the zero-shot baseline as measured by BERTScore F1,
and qualitatively reduces harmful outputs. The SFT baseline achieves lower perplexity,
consistent with DPO's objective of optimizing preference margins rather than token
likelihood. An ablation study over $\beta$, LoRA rank, and learning rate is underway.
Final scaled results (50k+ training samples) and preference accuracy metrics will be
reported in the camera-ready version. Code and results are available at
